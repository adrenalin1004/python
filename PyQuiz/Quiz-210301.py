...
#265 메서드, 종목명과 종목코드를 리턴하는 get_name, get_code 메서드를 추가하세요. 
...
삼성 = Stock5("삼성전자", "005930")
print(삼성.get_name(), 삼성.get_code())

#266 객체의 속성값 업데이트, 생성자에서 종목명, 종목코드, PER, PBR, 배당수익률을 입력 받을 수 있도록 생성자를 수정하세요. 
...
a = Stock6('삼성전자','005930',15.79,1.33,2.83)
print(a.배당수익률)

#268 객체의 속성 수정, PER, PBR, 배당수익률은 변경될 수 있는 값입니다. 
...
삼성 = Stock7("삼성전자","005930",15.79,1.33,2.83)
print(삼성.per)
삼성.set_per(12.75)
print(삼성.per)

#270 여러 종목의 객체 생성, 아래의 표를 참조하여 3종목에 대해 객체를 생성하고 이를 파이썬 리스트에 저장하세요. 
# 파이썬 리스트에 저장된 각 종목에 대해 for 루프를 통해 종목코드와 PER을 출력해보세요.

list = []

SS = Stock7("삼성전자","005930",15.79,1.33,2.83)
LG = Stock7("LG전자","005380",8.70,0.35,4.27)
HD = Stock7("현대차","005670",317.34,0.69,1.37)
...
data = []
k = Account1("KIM", 90000000)
l = Account1("LEE", 10000)
p = Account1("PARK", 10000)
